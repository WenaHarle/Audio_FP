...
def generate_spectrogram(samples, sample_rate):
    ...
    f, t, Zxx = stft(samples, fs=sample_rate, window=window, nperseg=1024, noverlap=512)
    return f, t, np.abs(Zxx)

def extract_peaks(spectrogram, threshold=0.5):
    peaks = ndi.maximum_filter(spectrogram, size=15) == spectrogram
    peaks &= spectrogram > np.mean(spectrogram) * threshold
    return np.where(peaks)

def generate_hashes(peak_freqs, peak_times, fan_value=25):
    hashes = []
...
def check_audio_segment_match(samples, sample_rate, original_hashes_path, threshold=0.009):
...
if __name__ == "__main__":
    try:
        ...
        original_hashes_path = "DataBase/Oleg6.json"
